...
#if [ -d "${XDG_DATA_HOME:-${HOME}/.local/share}/dotfiles" ]; then
if [ -d "${HOME}/.local/share/dotfiles" ]; then
	DEFAULT_DOTFILES_DIR="${HOME}/.local/share/dotfiles"
else
...
if [ -z "${RCRC}" ]; then
  if [ -d "${XDG_CONFIG_HOME:-${HOME}/.config}/rcm/" ]; then
