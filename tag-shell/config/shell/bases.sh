...
if [ -z "${XDG_CONFIG_HOME}" -a -d "${HOME}/._/${_shell_currenthost}/config" ]; then
	export XDG_CONFIG_HOME="${HOME}/._/${_shell_currenthost}/config"
else
	export XDG_CONFIG_HOME="${XDG_CONFIG_HOME:-${HOME}/.config}"
fi

if [ -z "${XDG_CACHE_HOME}" -a -d "${HOME}/._/${_shell_currenthost}/cache" ]; then
	export XDG_CACHE_HOME="${HOME}/._/${_shell_currenthost}/cache"
else
	export XDG_CACHE_HOME="${XDG_CACHE_HOME:-${HOME}/.cache}"
fi

if [ -z "${XDG_DATA_HOME}" -a -d "${HOME}/._/${_shell_currenthost}/share" ]; then
	export XDG_DATA_HOME="${HOME}/._/${_shell_currenthost}/share"
else
	export XDG_DATA_HOME="${XDG_DATA_HOME:-${HOME}/.local/share}"
fi

if [ -z "${XDG_STATE_HOME}" -a -d "${HOME}/._/${_shell_currenthost}/state" ]; then
	export XDG_STATE_HOME="${HOME}/._/${_shell_currenthost}/state"
else
...
_logging()
{
	...
	export SHELL_LOG="${XDG_STATE_HOME}/log/shell/$(date +%Y-%m-%d).log"
	touch "${SHELL_LOG}"
